...
export interface AppDataTableRow {
...
export const AppDataTable = ({ headers, data, selectedIndex = 0, onSelectRow }: AppDataTableProps) => {
              ...
              return (
                ...
                >
                  {Object.values(row).map((val, cellIndex) => (
                    <td key={`cell-${rowIndex}-${cellIndex}`} className="border-r border-b border-[#D4D0C8] px-2 py-0.5 whitespace-nowrap">
                      {String(val ?? '')}
                    </td>
                  ))}
                </tr>
    ...
    <div className="border border-slate-200 rounded-lg overflow-hidden bg-white shadow-xs">
      <table className="w-full text-sm text-left text-slate-600">
        <thead className="bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 uppercase">
          <tr>
            {headers.map((h, i) => (
              <th key={`${h}-${i}`} className="px-4 py-3">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {data.map((row, rowIndex) => (
            ...
            >
              {Object.values(row).map((val, cellIndex) => (
                <td key={`cell-${rowIndex}-${cellIndex}`} className="px-4 py-3 whitespace-nowrap">{String(val ?? '')}</td>
              ))}
            </tr>
          ))}
        </tbody>
